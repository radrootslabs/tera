...
enum TeraUserMessageClassifier {
  static func key(
    for error: Error,
  ...
  private static func runtimeKey(
    ...
    if error is TeraIdentityStoreError {
      return .identityOperationFailed
    }
    return nil
  }

  private static func identityCustodyKey(for error: Error) -> TeraUserMessageKey? {
    error is RadrootsIdentityCustodyError ? .secureStateUnavailable : nil
  }

  private static func platformKey(for error: Error) -> TeraUserMessageKey? {
  ...
  private static func runtimeClientKey(
    _ error: TeraRuntimeClientError,
    fallback: TeraUserMessageKey
  ) -> TeraUserMessageKey {
    switch error {

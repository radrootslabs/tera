...
struct TeraRemoteQualificationAuthorizationEvidence: Codable, Equatable {
...
struct TeraRemoteQualificationEvidenceStore {
  ...
  static func current(fileManager: FileManager = .default) throws -> Self? {
    guard let environment = try TeraRemoteQualificationEnvironment.current() else {
      return nil
    }
  ...
  func record(request: HostSigningRequest, signatureHex: String) throws {
    let evidence = TeraRemoteQualificationAuthorizationEvidence(
      runID: runID,
      request: request,
    ...
    guard data.count <= TeraRemoteQualificationAuthorizationEvidence.maximumEncodedBytes else {
...
enum TeraRemoteQualificationEvidence {
  static func prepare() throws -> TeraRemoteQualificationEvidenceStore? {
    try TeraRemoteQualificationEvidenceStore.cleanupLegacyAuthorization()
    let store = try TeraRemoteQualificationEvidenceStore.current()
    try store?.prepare()
    return store
  }
  ...
  static func recordBlossomAuthorization(
    ...
    guard try TeraRemoteQualificationEnvironment.current()?.networkMode
      == .isolatedLoopback
    else { return }
    try TeraRemoteQualificationEvidenceStore.current()?.record(
      request: request,
      signatureHex: signatureHex
    )
  }

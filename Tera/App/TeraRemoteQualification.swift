...
enum TeraQualificationNetworkMode: Sendable, Equatable {
...
struct TeraQualificationEndpoint: Sendable, Equatable {
  enum Role: Sendable {
    case relay
    case blossom
  }
  ...
  init(_ raw: String, role: Role, mode: TeraQualificationNetworkMode) throws {
    guard let value = URLComponents(string: raw),
      ...
      throw TeraConfigurationError.invalid("qualification_endpoint")
    }
    ...
    guard scheme == expectedScheme, validHost else {
      throw TeraConfigurationError.invalid("qualification_endpoint_policy")
    }
    rawValue = raw
  }
}

struct TeraRemoteQualificationEnvironment: Sendable, Equatable {
  static let enabledKey = "TERA_IOS_UI_TEST_REMOTE"
  static let runIDKey = "TERA_IOS_UI_TEST_RUN_ID"
  static let relayURLsKey = "TERA_IOS_UI_TEST_NOSTR_RELAY_URLS"
  static let blossomOriginsKey = "TERA_IOS_UI_TEST_BLOSSOM_ORIGINS"
  static let mediaRelativePathKey = "TERA_IOS_UI_TEST_MEDIA_RELATIVE_PATH"
  static let networkProfileKey = "TERA_IOS_UI_TEST_NETWORK_PROFILE"

  let runID: String
  let relayURLs: [String]
  let blossomOrigins: [String]
  let mediaFile: RadrootsFileReference?
  let networkMode: TeraQualificationNetworkMode
  ...
  static func mediaFixtureData() throws -> Data {
    guard let data = Data(base64Encoded: mediaFixtureBase64), !data.isEmpty else {
      throw TeraConfigurationError.invalid("qualification_media_fixture")
    }
    return data
  }
    ...
    static func current(
      ...
      let networkMode = try TeraQualificationNetworkMode(
        profile: environment[networkProfileKey]
      )
      guard !relays.isEmpty, blossoms.count == 1 else {
        throw TeraConfigurationError.invalid("qualification_blossom_origin")
      }
      let relayEndpoints = try relays.map {
        try TeraQualificationEndpoint($0, role: .relay, mode: networkMode)
      }
      let blossomEndpoints = try blossoms.map {
        try TeraQualificationEndpoint($0, role: .blossom, mode: networkMode)
      }
      guard Set(relayEndpoints.map(\.rawValue)).count == relayEndpoints.count else {
        throw TeraConfigurationError.invalid("qualification_endpoint_duplicate")
      }
      let mediaFile = try environment[mediaRelativePathKey].map { raw in
        guard raw == "qualification/input.png" else {
          throw TeraConfigurationError.invalid("qualification_media_file")
        }
    ...
    private static func requiredRunID(_ raw: String?) throws -> String {
      guard let raw,
        ...
        throw TeraConfigurationError.invalid("qualification_run_id")
      }
      return raw
    }
...
#if DEBUG
  final class TeraRemoteQualificationUserPresence: RadrootsUserPresence, Sendable {
    init(mode: TeraQualificationNetworkMode) throws {
      guard mode.permitsAutomatedUserPresence, mode.permitsTestSecretPolicy else {
        throw TeraConfigurationError.invalid("qualification_user_presence")
      }
    }

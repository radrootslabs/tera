...
enum TeraAppNetworkProfile: String, Codable, Sendable, Equatable {
    case publicNetwork = "public"
    case simulator
    case device

    var runtimeValue: TeraRuntimeNetworkProfile {
        switch self {
...
struct TeraAppConfiguration: Sendable, Equatable {
    let profile: TeraAppNetworkProfile
    let writableRelays: [String]
    let blossom: TeraBlossomEndpointConfiguration?
    let keychainServicePrefix: String
    let bundleIdentifier: String
    let appMetadata: TeraRuntimeAppMetadata
    let generation: UInt64
    let activationState: TeraConfigurationActivationState
    let previousBlossomConfigFingerprint: String?
}

enum TeraConfigurationActivationState: String, Codable, Sendable, Equatable {
    case current
    case reconfigurationRequired = "reconfiguration_required"
}

struct TeraConfigurationBootstrap: Sendable, Equatable {
...
struct TeraSourceGeneration: Codable, Sendable, Equatable {
    let schemaVersion: UInt16
    let generationHex: String
    let createdAtUnixMilliseconds: UInt64
}

enum TeraConfigurationError: Error, Sendable, Equatable {
    case missing(String)
    case invalid(String)
...
extension TeraConfigurationError: LocalizedError {
    var errorDescription: String? {
        switch self {
        case .missing:
            "Required Tera configuration is missing."
        case .invalid:
            "Tera network configuration is invalid."
        case .corruptStoredConfiguration:
            "Stored Tera settings are corrupt and require recovery."
        case .corruptSourceGeneration:
            "Stored Tera local-state identity is corrupt and requires recovery."
        case .persistenceFailed:
            "Tera could not persist its local configuration."
        }
    }
}

actor TeraConfigurationStore {
    ...
    private let bootstrap: TeraConfigurationBootstrap
    ...
    func load() throws -> TeraAppConfiguration {
        ...
        if let stored = try readStoredConfiguration() {
            guard stored.format == StoredConfigurationV3.format else {
                throw TeraConfigurationError.corruptStoredConfiguration
            }
        ...
        let relays = try TeraNetworkValidator.relays(
        ...
        guard !bootstrap.keychainServicePrefix.isEmpty,
            ...
            throw TeraConfigurationError.missing("identity")
        }
        return TeraAppConfiguration(
          profile: profile,
    ...
    func confirmCanonicalBlossomConfiguration(
      _ canonical: TeraBlossomConfigurationStatus?,
      expectedGeneration: UInt64
        ...
        let canonicalBlossom: TeraBlossomEndpointConfiguration?
        ...
        case let (.some(selected), .some(canonical)):
            guard let hostKind = TeraBlossomHostKind(rawValue: canonical.hostKind),
                ...
                throw TeraConfigurationError.invalid("canonical_blossom_configuration")
            }
            canonicalBlossom = TeraBlossomEndpointConfiguration(
              hostKind: hostKind,
    ...
    func sourceGeneration() throws -> TeraSourceGeneration {
        if let data = try read(Self.sourceGenerationFile) {
            guard let value = try? JSONDecoder().decode(TeraSourceGeneration.self, from: data),
                ...
                throw TeraConfigurationError.corruptSourceGeneration
            }
            return value
        }
        ...
        let value = TeraSourceGeneration(
          schemaVersion: 1,
    ...
    private static func nextGeneration(after generation: UInt64) throws -> UInt64 {
        do {
            return try TeraCheckedStateTransition.nextGeneration(after: generation)
        } catch {
            throw TeraConfigurationError.corruptStoredConfiguration
        }
    }

    private static func profile(for runtimeMode: String) throws -> TeraAppNetworkProfile {
    ...
    private static func bootstrapConfiguration(
      _ bootstrap: TeraConfigurationBootstrap,
      profile: TeraAppNetworkProfile,
      generation: UInt64,
      activationState: TeraConfigurationActivationState,
    ...
    private static func bootstrapFingerprint(
      ...
      profile: TeraAppNetworkProfile
        ...
        do {
            ...
            throw TeraConfigurationError.invalid("bootstrap_network_identity")
        }
    }
    ...
    private static func blossomConfiguration(
      ...
      profile: TeraAppNetworkProfile
        ...
        return TeraBlossomEndpointConfiguration(
    ...
    private static func blossomMatchesProfile(
      _ configuration: TeraBlossomEndpointConfiguration?,
      profile: TeraAppNetworkProfile
    ) -> Bool {
    ...
    private static func blossomIdentity(
      ...
      hostKind: TeraBlossomHostKind,
      endpointAuthority: TeraBlossomEndpointAuthority
    ) {
        switch profile {
        case .publicNetwork:
      ...
      profile: TeraAppNetworkProfile
        ...
        for raw in values {
            ...
            guard seen.insert(canonical).inserted else {
                throw TeraConfigurationError.invalid("duplicate_relay")
            }
            output.append(canonical)
        }
        if profile != .publicNetwork, output.isEmpty {
            throw TeraConfigurationError.invalid("empty_relay_set")
        }
        let totalCount = output.count + (profile == .simulator ? 0 : 1)
        guard totalCount <= 64 else {
            throw TeraConfigurationError.invalid("too_many_relays")
        }
        return output
    }
    ...
    private static func relay(
      ...
      profile: TeraAppNetworkProfile
        ...
        guard raw == raw.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            throw TeraConfigurationError.invalid("relay_url")
        }
        ...
        guard schemeAllowed, hostAllowed(host, profile: profile) else {
            throw TeraConfigurationError.invalid("relay_policy")
        }

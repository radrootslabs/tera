...
mod tests {
    ...
    async fn kind_one_revision_never_creates_retraction_before_replacement_acceptance() {
        ...
        let target = retraction_support::retain_original(&runtime, &command).await;
    ...
    async fn revision_coordinator_reopens_from_sqlite_without_losing_ordering() {
        ...
        let target = retraction_support::retain_original(
            &runtime,
            &Phase1AddCommand::CreateAsk(
                CreateAsk::new("Who has seed potatoes?", Vec::new()).unwrap(),
            ),
        )
        .await;
        let saved = runtime
    ...
    async fn retraction_is_independent_and_add_advance_attempts_delivery() {
        ...
        let original = retraction_support::original(&runtime).await;
        let target = original.card_id();

...
final class TeraRemoteQualificationUITests: XCTestCase {
  ...
  func testRemoteBlossomUploadAndRecovery() throws {
    let configuration = try QualificationConfiguration.environment()
    let app = launchToRoot(configuration)
    let marker = "Tera remote qualification \(configuration.runID)"
  ...
  func testRemoteBlossomUnavailablePersistsDraft() throws {
    let configuration = try QualificationConfiguration.environment()
    let app = launchToRoot(configuration)
    let marker = "Tera unavailable qualification \(configuration.runID)"
  ...
  func testManualDeviceLockAndProtectedDataRecovery() throws {
    ...
    XCTFail("Tera did not recover after the physical device was unlocked")
  }
  ...
  private func loadPersonaSuite() throws -> PersonaSuite {
    let bundle = Bundle(for: TeraRemoteQualificationUITests.self)
  ...
  private func activateFixture(persona: String, at path: String) throws {
    ...
    let data = try JSONSerialization.data(
      withJSONObject: [
        "schema": "tera.ios.local-social.persona-control.v1",
  ...
  private func writeBootstrapReceipt(
    ...
    let receipt = BootstrapReceipt(
      schema: "tera-ios-remote-qualification-bootstrap-v1",
      schemaVersion: 1,
  ...
  private func writePersonaAttemptAttachment(
    ...
    let evidence = PersonaAttemptEvidence(
      schema: "tera.ios.local-social.persona-attempt-evidence.v1",
      schemaVersion: 1,
      testInvocation: .init(
        target: "TeraUITests",
        identifier: "TeraUITests/testLocalSocialDeterministicPersonas",
        action: "test",
        configuration: "Debug"
      ),
...
private struct QualificationConfiguration {
  ...
  static let fixtureControlKey = "TERA_IOS_UI_TEST_FIXTURE_CONTROL"
  static let fixtureEvidenceKey = "TERA_IOS_UI_TEST_FIXTURE_EVIDENCE"
  ...
  static let sourceCommitKey = "TERA_IOS_UI_TEST_SOURCE_COMMIT"
  static let sourceTreeKey = "TERA_IOS_UI_TEST_SOURCE_TREE"
  static let appBuildSHA256Key = "TERA_IOS_UI_TEST_APP_BUILD_SHA256"
  static let simulatorIDKey = "TERA_IOS_UI_TEST_SIMULATOR_ID"
...
private struct PersonaFixtureNetworkSnapshot {
  ...
  init(_ value: [String: Any]) throws {
    guard Set(value.keys) == Self.keys,
      value["schema"] as? String == "tera.ios.local-social.persona-evidence.v1",

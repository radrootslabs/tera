...
final class TeraLifecycleTests: XCTestCase {
  ...
  func testBackgroundEventsCompleteExactlyOnceBeforeAndAfterAttachment() async {
    let router = TeraBackgroundEventRouter()
    let first = CompletionProbe()

    await router.handle(
      identifier: "org.radroots.tests.background.transfer",
      completion: TeraCompletionOnce { first.increment() }
    )
    ...
    await router.handle(
      ...
      completion: TeraCompletionOnce { mismatched.increment() }
    )
  ...
  func testBackgroundEventQueueIsBoundedAndShutdownCompletesPendingEvents() async {
    ...
    for probe in probes {
      await router.handle(
        identifier: "org.radroots.tests.background.transfer",
        completion: TeraCompletionOnce { probe.increment() }
      )
    }
  ...
  func testProtectedDataMonitorReflectsTransitions() {
    let monitor = TeraProtectedDataMonitor(available: false)
  ...
  func testDiagnosticsAreBoundedAndExcludeSensitiveRuntimeValues() async throws {
    ...
    let coordinator = TeraLifecycleCoordinator.testing(roots: roots, capacity: 16)

...
final class TeraTodayReconciliationFFITests: XCTestCase {
  func testGeneratedReconciliationCarriesCurrentGenerationAndRejectsInvalidSelection() async throws {
    ...
    let page = try await runtime.phase1TodayPage(
      context: context, limit: 20, asOfUnixS: 1_900_000_000, cursor: nil
    )
    let current = try await runtime.phase1TodayReconcile(
      context: context, asOfUnixS: page.asOfUnixS, cardIds: [],
      expectedGeneration: page.projectionGeneration
    )
    ...
    do {
      _ = try await runtime.phase1TodayReconcile(
        context: context, asOfUnixS: page.asOfUnixS,
        cardIds: [String(repeating: "a", count: 65)], expectedGeneration: nil
      )

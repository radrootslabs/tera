...
final class TeraTodayStoreTests: XCTestCase {
    @MainActor
    ...
    func testFiveCardsExposeProfileThreadMediaOverlayAndAccessibleProductFields() {
        ...
        let failedMedia = TeraMediaReference(
        ...
        let cards = TeraTodayCardType.allCases.map { type in
            makeCard(
              id: type.rawValue,
              type: type,
              profile: profile,
        ...
        XCTAssertEqual(cards.map(\.type), TeraTodayCardType.allCases)
    ...
    @MainActor
    private func startedClient(backend: TodayBackend) async throws -> TeraRuntimeClient {
...
private func makeContext(id: String, label: String) -> TeraLocalNetwork {
    TeraLocalNetwork(
      schemaVersion: 1,
      id: id,
      label: label,
...
private func makeCard(
  id: String,
  type: TeraTodayCardType,
  authoredAt: UInt64 = 1_800_000_000,
  profile: TeraProfileSummary? = nil,
  media: [TeraMediaReference] = [],
  thread: [TeraThreadEntry] = [],
...
private struct TodayTestSigner: TeraRuntimeSigner {
...
private actor TodayBackend: TeraRuntimeBackend {
    private let pages: [String: TeraTodayPage]
    private let delays: [String: UInt64]
    private let refreshFailure: TeraRuntimeFailure?
    private var closed = false

    init(
      pages: [String: TeraTodayPage],
      delays: [String: UInt64] = [:],
      refreshFailure: TeraRuntimeFailure? = nil
    ) {
        self.pages = pages
        self.delays = delays
...
private actor TodaySubscriptionToken: TeraRuntimeSubscriptionToken {
    func cancel() {}
}

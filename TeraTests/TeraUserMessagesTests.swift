...
final class TeraUserMessagesTests: XCTestCase {
  func testMessageKeyInventoryIsClosedAndLocalized() {
    XCTAssertEqual(
      Set(TeraUserMessageKey.allCases.map(\.rawValue)),
      [
        ...
        "error.operation.outcome_unknown",
    ...
    for key in TeraUserMessageKey.allCases {
      XCTAssertFalse(TeraUserMessages.text(key).isEmpty, key.rawValue)
    }
  }

  func testAppleErrorsMapToClosedUserMessageKeys() {
    let vectors: [(Error, TeraUserMessageKey)] = [
    ...
    for (error, expected) in vectors {
      XCTAssertEqual(
        TeraUserMessages.key(for: error, fallback: .runtimeOperationFailed),
        expected
      )
    }
  }
  ...
  func testDependencyDiagnosticsAndRuntimeSafeMessagesCannotReachUserText() {
    ...
    let errors: [Error] = [
      ...
      TeraRuntimeFailure.local(
        operation: canaries[2],
        code: "private.failure",
        safeMessage: canaries[0]
      ),
      TeraConfigurationError.missing(canaries[1]),
      TeraIdentityStoreError.custody(canaries[0]),
    ]

    for error in errors {
      let rendered = TeraUserMessages.text(for: error, fallback: .runtimeOperationFailed)

...
final class TeraRemoteQualificationTests: XCTestCase {
  func testQualificationIsOptInAndCarriesOnlyPublicInputs() throws {
    XCTAssertNil(try TeraRemoteQualificationEnvironment.current(environment: [:]))

    let value = try XCTUnwrap(
      TeraRemoteQualificationEnvironment.current(
        environment: [
          TeraRemoteQualificationEnvironment.enabledKey: "1",
          TeraRemoteQualificationEnvironment.runIDKey: "gbrqv1-12345678",
          TeraRemoteQualificationEnvironment.relayURLsKey: "wss://write.example",
          TeraRemoteQualificationEnvironment.blossomOriginsKey: "https://media.example",
          TeraRemoteQualificationEnvironment.mediaRelativePathKey: "qualification/input.png",
          TeraRemoteQualificationEnvironment.networkProfileKey: "public",
        ]
      )
    )
  ...
  func testQualificationRejectsAmbiguousOrUnsafeHarnessValues() {
    ...
    XCTAssertThrowsError(
      try TeraRemoteQualificationEnvironment.current(
        environment: base.merging([
          TeraRemoteQualificationEnvironment.runIDKey: "../../bad",
        ]) { _, new in new }
      )
    )
    for endpoint in [
    ...
    XCTAssertThrowsError(
      try TeraRemoteQualificationEnvironment.current(
        environment: base.merging([
          TeraRemoteQualificationEnvironment.networkProfileKey: "simulator",
          TeraRemoteQualificationEnvironment.relayURLsKey:
            "ws://127.0.0.1:21000,ws://127.0.0.1:21000",
          TeraRemoteQualificationEnvironment.blossomOriginsKey:
    ...
    XCTAssertThrowsError(
      try TeraRemoteQualificationEnvironment.current(
        environment: base.merging([
          TeraRemoteQualificationEnvironment.networkProfileKey: "automatic",
        ]) { _, new in new }
      )
    )

    let simulator = try? TeraRemoteQualificationEnvironment.current(
      environment: base.merging([
        TeraRemoteQualificationEnvironment.networkProfileKey: "simulator",
        TeraRemoteQualificationEnvironment.relayURLsKey: "ws://127.0.0.1:21000",
        TeraRemoteQualificationEnvironment.blossomOriginsKey:
          "http://127.0.0.1:21100",
      ]) { _, new in new }
    )
    ...
    XCTAssertThrowsError(
      try TeraRemoteQualificationEnvironment.current(
        environment: base.merging([
          TeraRemoteQualificationEnvironment.mediaRelativePathKey:
            "../../private-key",
        ]) { _, new in new }
      )
    )
  ...
  func testQualificationUserPresenceIsNoninteractiveAndDebugOnly() async throws {
    let presence = try TeraRemoteQualificationUserPresence(mode: .isolatedLoopback)
  ...
  func testPublicQualificationCannotSelectAutomatedPresence() throws {
    let publicMode = TeraQualificationNetworkMode.publicEndpoint
    XCTAssertFalse(publicMode.permitsAutomatedUserPresence)
    XCTAssertFalse(publicMode.permitsTestSecretPolicy)
    XCTAssertThrowsError(
      try TeraRemoteQualificationUserPresence(mode: publicMode)
    )
  }
  ...
  func testQualificationModeRejectsMutationsWithoutRenderingInput() {
    ...
    for profile in [nil, "", "SIMULATOR", "public ", "automatic", canary] {
      XCTAssertThrowsError(try TeraQualificationNetworkMode(profile: profile)) { error in
  ...
  func testQualificationUsesRunScopedRootsAndBackgroundSession() throws {
    ...
    let qualification = try XCTUnwrap(
      TeraRemoteQualificationEnvironment.current(
        environment: [
          TeraRemoteQualificationEnvironment.enabledKey: "1",
          TeraRemoteQualificationEnvironment.runIDKey: "persona-p01-12345678",
          TeraRemoteQualificationEnvironment.relayURLsKey:
            "ws://127.0.0.1:21000",
          TeraRemoteQualificationEnvironment.blossomOriginsKey:
            "http://127.0.0.1:21100",
  ...
  func testQualificationMediaFixtureHasPinnedDigest() throws {
    let digest = try SHA256.hash(
      data: TeraRemoteQualificationEnvironment.mediaFixtureData()
    )
    XCTAssertEqual(
  ...
  func testAuthorizationEvidenceIsRedactedEphemeralAndRelaunchCleaned() throws {
    ...
    let store = TeraRemoteQualificationEvidenceStore(
    ...
    XCTAssertLessThanOrEqual(
      data.count,
      TeraRemoteQualificationAuthorizationEvidence.maximumEncodedBytes
    )
  ...
  func testAuthorizationEvidenceDigestIsDomainSeparatedAndFieldSensitive() {
    ...
    let baseline = TeraRemoteQualificationAuthorizationEvidence(
      runID: "persona-p01-12345678",
      request: request,
      signatureHex: String(repeating: "b", count: 128)
    )
    let changedRun = TeraRemoteQualificationAuthorizationEvidence(
      runID: "persona-p02-12345678",
      request: request,
      signatureHex: String(repeating: "b", count: 128)
    )
    let changedSignature = TeraRemoteQualificationAuthorizationEvidence(
      runID: "persona-p01-12345678",
      request: request,
    ...
    XCTAssertTrue(
      TeraRemoteQualificationAuthorizationEvidence.digestDomain.hasSuffix("\0")
    )
  }

  func testAuthorizationEvidenceWriteFailureFailsSigningResult() async {
    let signer = TeraGeneratedHostSigner(
...
private struct QualificationSignedSigner: TeraRuntimeSigner {
